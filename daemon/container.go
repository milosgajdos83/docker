...
func (container *Container) Copy(resource string) (io.ReadCloser, error) {
	...
	for _, mnt := range container.volumeMounts() {
		if len(mnt.containerPath) > 0 && strings.HasPrefix(resource, mnt.containerPath[1:]) {
			if v := container.daemon.volumes.Get(mnt.hostPath); v != nil {
				var name string
				if resource == mnt.containerPath[1:] {
					name = filepath.Base(resource)
				}
				path, err := filepath.Rel(mnt.containerPath[1:], resource)
				if err != nil {
					return nil, err
				}
